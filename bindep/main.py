...
class Bindep(object):
    ...
    def _list_all_packages(self, rules):
        rules = self._get_rules()
        self.__depends.list_all_packages(rules, self.__opts.list_all)
        return 0

    def _get_rules(self):
        profiles = self.__args if self.__args else ['default']
        profiles = profiles + self.__depends.platform_profiles()
        return self.__depends.active_rules(profiles)
    ...
    def run(self):
        ...
        elif self.__opts.list_all:
            return self.__list_all_packages()
